...
{
  if (narg < 7) error->all(FLERR,"Illegal fix bond/break command");
  ...
  cutoff_min = force->numeric(FLERR,arg[5]);
  cutoff = force->numeric(FLERR,arg[6]);

  if (btype < 1 || btype > atom->nbondtypes)
    error->all(FLERR,"Invalid bond type in fix bond/break command");
  if (cutoff < 0.0 || cutoff_min<0.0) error->all(FLERR,"Illegal fix bond/break command");

  cutsq = cutoff*cutoff;
  cutsq_min = cutoff_min*cutoff_min;
...
void FixDoublesBondBreak::post_integrate()
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if (rsq <= cutsq && rsq >= cutsq_min ) continue;
    //FILE * pFile;
    //pFile = fopen("/home/noahbensler/pure-summer-2020/src/myfile.txt","w");
    //fprintf(pFile,"rsq: %d cutsq: %d",rsq,cutsq);
    //fclose(pFile);
    //std::cout << n << " rsq: " << rsq << " cutsq: " << cutsq << " distsq[i1]: " << distsq[i1] << " distsq[i2]: " << distsq[i2] << "\n";
