...
#include "fix_doubles_bond_break.h"
#include <iostream>
#include <mpi.h>
#include <cstring>
#include <stdio.h>
...
FixDoublesBondBreak::FixDoublesBondBreak(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  partner(NULL), finalpartner(NULL), distsq(NULL), probability(NULL),
  broken(NULL), copy(NULL), random(NULL), id_fix(NULL)
{
  if (narg < 5) error->all(FLERR,"Illegal fix doubles/bond/break command");

  MPI_Comm_rank(world,&me);
  MPI_Comm_size(world,&nprocs);

  nevery = force->inumeric(FLERR,arg[3]);
  if (nevery <= 0) error->all(FLERR,"Illegal fix doubles/bond/break command");
...
//    error->all(FLERR,"Invalid bond type in fix bond/break command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"prob") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix doubles/bond/break command");
      fraction = force->numeric(FLERR,arg[iarg+1]);
      seed = force->inumeric(FLERR,arg[iarg+2]);
      if (fraction < 0.0 || fraction > 1.0)
        error->all(FLERR,"Illegal fix doubles/bond/break command");
      if (seed <= 0) error->all(FLERR,"Illegal fix doubles/bond/break command");
      iarg += 3;
    } else error->all(FLERR,"Illegal fix doubles/bond/break command");
  }
  ...
  // only write if nbroken > 0
  update->WRITE_FFLAG = 0;
  ...
  //--- store reference coordinates 
	...
	int index_i;
	//assert nlocal is global
    tagint maxtag = 0;
    for (int i = 0; i < nall; i++) maxtag = MAX(atom->tag[i],maxtag); //--- largest atom tag
    tagint maxtag_all;
    MPI_Allreduce(&maxtag,&maxtag_all,1,MPI_LMP_TAGINT,MPI_MAX,world);
    maxtag = maxtag_all;
//	std::cout << "me="<< me <<",maxtag=" << maxtag << "\n";
//	std::cout <<"update->ntimestep=" << update->ntimestep << "\n";
    memory->create( xorigin, maxtag + 1, 3, "bond/break:xorigin"  );
    for (int i = 0; i < nall; i++){
			index_i = tag[i];
			assert( index_i < maxtag + 1 && "assertion failed" );
      		xorigin[index_i][0] = x[i][0];
			xorigin[index_i][1] = x[i][1];
			xorigin[index_i][2] = x[i][2];
	  }

 //--- file: output broken bonds 
	sfile = fopen("BrokenBonds.dump","w"); 
}

/* ---------------------------------------------------------------------- */

FixDoublesBondBreak::~FixDoublesBondBreak()
{
  delete random;

  memory->destroy( xorigin );
...
int FixDoublesBondBreak::setmask()
{
  int mask = 0;
  mask |= POST_INTEGRATE;
...
void FixDoublesBondBreak::post_integrate()
{
  int i,j,k,m,n,i1,i2,n1,n3,type;
  double delx,dely,delz,rsq, rsq0, delx0,dely0,delz0;
  tagint *slist;
  update->WRITE_FFLAG = 0;

//  if (update->ntimestep != 1 && update->ntimestep % nevery) return;
  ...
  int index_i, index_j;
//  std::cout << "update->ntimestep =" << update->ntimestep<<"\n";  
//  if(me==0) std::cout << "nall="<< nall << "\n";
//  if( update->ntimestep == 1 ){ //--- set initial length
//	if(me==0) std::cout << "init: tag[0]=" << tag[0] << "\n"; 
//    memory->create( xorigin, nall+1, 3, "bond/break:xorigin"  );
...
//  }
	//assert nlocal is global
// if( me == 0) std::cout << "break: me="<< me<<",natom=" << nlocal << "atom->nmax="<< atom->nmax <<"\n";
// if( me == 0)  std::cout << "me="<< me<<",nbondlist=" << nbondlist <<"\n";
//  if(me==0) std::cout << "tag[0]=" << tag[0] << "\n"; 
...
//    if (type != btype) continue;
	...
	//
//	std::cout << "nlocal="<< nlocal<<"i,j=" <<i1 << "," << i2 << "\n";
//	assert( i1 < atom->nmax && i2 < atom->nmax && "asertion failed\n" );
 	index_i = tag[i1];
	index_j = tag[i2];
    delx0 = xorigin[index_i][0] - xorigin[index_j][0];
    dely0 = xorigin[index_i][1] - xorigin[index_j][1];
    delz0 = xorigin[index_i][2] - xorigin[index_j][2];
    rsq0 = delx0*delx0 + dely0*dely0 + delz*delz0;
//	if( update->ntimestep == 1 ) //--- set initial length
//	{
//		rsq0[ n ] = rsq;
//		assert( rsq0[ n ] > 0.0 && "set?????initial length is not set!" );
//	}
//	if( rsq0[ n ] <= 0.0 )
//		std::cout << i1 <<"\t"<< i2 << "\t"<< rsq0[ n ] << "\n";
//	assert( rsq0[ n ] > 0.0 && "initial length is not set!" );
	double epsilon = pow( rsq / rsq0, 0.5 ) - 1.0; //--- compute strain
	
//	epsilon_max = MAX(fabs(epsilon),epsilon_max); //--- largest atom tag
//	if( update->ntimestep != 1)	
//	std::cout << "epsilon_max="<<epsilon_max<<"\n";
//    tagint maxtag_all;
 //   MPI_Allreduce(&maxtag,&maxtag_all,1,MPI_LMP_TAGINT,MPI_MAX,world);
//    maxtag = maxtag_all;
//	if( tag[i1]==1063 && tag[i2]==2636) std::cout << "i1="<<i1<<",i2="<<i2<<",epsilon="<<fabs(epsilon) <<",rsq0="<<rsq0<<"\n";
//	if ( fabs( epsilon ) <= cutoff ) continue;
	if ( epsilon  <= cutoff ) continue; //--- only breaks in tension
//	std::cout << "ntimestep="<< update->ntimestep<< ",index_i="<< index_i <<"index_j="<< index_j <<"rsq=" << rsq<<",rsq0="<<rsq0<< "\n";
...
//  tagint **broken_bonds = atom->broken_bonds;
...
//	if( world_rank==1)
//		std::cout << "world_rank="<<world_rank<<" nbreak_before="<<nbreak-breakcount<<"\n";
...
//	  std::cout << "i="<< tag[i] << "j=" << finalpartner[i] << "\n"; 
    }
  }
  ...
  // also remove angles/dihedrals/impropers broken by broken bonds
	assert(nbreak == breakcount); //--- array "broken" includes all broken bonds from each process
	update->WRITE_FFLAG = ( nbreak > 0 );
//	if( world_rank==1)
//		std::cout << "world_rank="<<world_rank<<" nbreak_after="<<nbreak-breakcount<<"\n";
  	update_topology();
...
void FixDoublesBondBreak::check_ghosts()
{
  int i,j,n;
  tagint *slist;
...
void FixDoublesBondBreak::update_topology()
{
...
// store broken bonds in two arrays
//  int* broken_i = new int( nbreak );
//  int* broken_j = new int( nbreak );
//  for (i = 0; i < nbreak; i++){
//   broken_i[ i ] = broken[i][0];
//   broken_j[ i ] = broken[i][1];
//  }
...
//	std::cout << "world_rank="<<world_rank<<" nbreak="<<nbreak<<"\n";
   int global_nbreak;
   MPI_Reduce(&nbreak, &global_nbreak, 1, MPI_INTEGER, MPI_SUM, 0,
            world);  

//--- *gather* nbreak
  int* nbreak_array = new int( gsize );
  MPI_Allgather( &nbreak, 1, MPI_INTEGER, nbreak_array, 1, MPI_INTEGER, world );



//--- allocate global array
  int* broken_i_glob = NULL;
  int* broken_j_glob = NULL;
  if( world_rank == 0 ){ // retains the order???
  	broken_i_glob = new int( global_nbreak );
  	broken_j_glob = new int( global_nbreak );
 }
...
//--- gather arrays from all processes
  ...
  MPI_Gatherv( broken_i, nbreak, MPI_INT, broken_i_glob, nbreak_array, displs, MPI_INT, 0, world  );
  MPI_Gatherv( broken_j, nbreak, MPI_INT, broken_j_glob, nbreak_array, displs, MPI_INT, 0, world  );
//  MPI_Gather( broken_i, nbreak, MPI_INT, broken_i_glob, nbreak, MPI_INT, 0, world); 
//  MPI_Gather( broken_j, nbreak, MPI_INT, broken_j_glob, nbreak, MPI_INT, 0, world); 
*/
//--- rank 0: write to disk 
  if( world_rank == 0 ){
//	fprintf(sfile, "ITEM: ITIME\n%d\nITEM: NBREAK\n%d\nITEM: BOND_i\tBOND_j\n",update->ntimestep,global_nbreak);
	fprintf(sfile, "ITEM: ITIME\n%d\nITEM: NBREAK\n%d\nITEM: BOND_i\tBOND_j\n",update->ntimestep,nbreak);
//  	for(i = 0; i < global_nbreak; i++)
  	for(i = 0; i < nbreak; i++){
//		fprintf(sfile, " %d %d\n",broken_i_glob[i],broken_j_glob[i]);
//		int ii = broken[i][2];
//		int jj = broken[i][3];
		fprintf(sfile, " %d\t%d\n",
				broken[i][0],broken[i][1]);
	}
  }
  fflush( sfile );
/*
  ...
  delete [] nbreak_array;
  if( world_rank == 0 ){
	delete [] broken_i_glob;
	delete [] broken_j_glob;
  }	
*/
...
void FixDoublesBondBreak::rebuild_special_one(int m)
{
...
void FixDoublesBondBreak::break_angles(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixDoublesBondBreak::break_dihedrals(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixDoublesBondBreak::break_impropers(int m, tagint id1, tagint id2)
{
  int j,found;
...
int FixDoublesBondBreak::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
...
void FixDoublesBondBreak::post_integrate_respa(int ilevel, int /*iloop*/)
{
...
int FixDoublesBondBreak::pack_forward_comm(int n, int *list, double *buf,
...
void FixDoublesBondBreak::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixDoublesBondBreak::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDoublesBondBreak::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDoublesBondBreak::print_bb()
{
...
void FixDoublesBondBreak::print_copy(const char *str, tagint m,
...
double FixDoublesBondBreak::compute_vector(int n)
{
...
double FixDoublesBondBreak::memory_usage()
{
  int nmax = atom->nmax;
