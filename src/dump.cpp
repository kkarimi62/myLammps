...
//   std::cout << "ntimestep,WRITE_FFLAG="<<update->ntimestep<<"\t"<<update->WRITE_FFLAG<<"\n";
  if( not update->WRITE_FFLAG ){ //--- added by KK
  	if (refreshflag) modify->compute[irefresh]->refresh();
	update->vflag_atom = update->ntimestep;	return; //kam
	return;
  }
  update->vflag_atom = update->ntimestep; //--- added by KK
